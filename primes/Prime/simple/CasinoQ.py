...
def lisa_reaction():
    ...
    new_guess = generate_valid_hex_string(7)
...
def casino_hex_prime(max_iterations=2000, failure_threshold=500):
    ...
    for iteration in range(max_iterations):  # Exécuter la boucle max_iterations fois
        ...
        while True:
            ...
            random_hex_string = generate_valid_hex_string(7)

            # Vérifier le 261ᵐ caractère (index 260 en Python)
            middle_char = random_hex_string[3]

            if middle_char in "123":
                print(f"Tentative ignorée: {random_hex_string}")
                print(
                    f"Tentative ignorée car middle char est : {random_hex_string[1:3]}+<{middle_char}>+{random_hex_string[4:6]}")
                failure_count += 1
        ...
        # Vérifier les deux versions : avec 0 et avec 7
            hex_with_0 = random_hex_string[:3] + "0" + random_hex_string[4:]
            hex_with_7 = random_hex_string[:3] + "7" + random_hex_string[4:]
